...
- Quick win / low-hanging fruit (e.g., `[lightning] Quick Wins`, `easy fix`)
- Help wanted / good first issue (e.g., `[hand]help wanted`, `[Y] good first issue`)
- Regression labels (e.g., `[chart] regressing`, `regression`) -- maintainers want these fixed urgently
...
4. If either level finds >2 competing PRs actually targeting the same fix -> mark as [N].
...
```
 ...
 1  #XXXXX   [Y] reason     [Y]        [Y]      description
 2  #YYYYY   [Y] reason     [N] 3 PRs  [Y]      description
```
